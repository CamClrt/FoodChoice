...
class ProductManager:
    ...
    def insert(self, product_object):
        ...
        mycursor = self.database.cursor()
        mycursor.execute(queries.SQL_INSERT_PRODUCTS, data)
    ...
    def find_and_display_by_category(self):
        ...
        most_used_categories = sorted(
            tmp_most_used_categories, key=lambda x: x[0]
        )
        ...
        while cnx:
            ...
            if category_choice in categories.keys():
                ...
                mycursor = self.database.cursor()
                mycursor.execute(
                    queries.SQL_SELECT_PRODUCTS_BY_CATEGORY, (category_id,)
                )
                ...
                for tmp_index, product_by_category in enumerate(
                    products_by_category
                ):
                    index = tmp_index + 1
                    products[str(index)] = product_by_category
                    print(
                        f"{str(index)[:10].center(10)}|"
                ...
                print(
                    f"\n '{category_choice}': "
                    f"cette catégorie ne figure pas dans la liste\n"
                )
    ...
    def find_and_display_by_name(self, tmp_product_name):
        ...
        mycursor.execute(queries.SQL_SELECT_PRODUCTS_BY_NAME, (product_name,))
        sql = mycursor.statement
    ...
    def display_product(self, product_id):
        ...
        mycursor.execute(queries.SQL_SELECT_PRODUCT, (product_id,))
        product = mycursor.fetchone()

        mycursor.execute(queries.SQL_SELECT_CATEGORY_PRODUCT, (product_id,))
        categories_res = mycursor.fetchall()
        categories = []
        for category_res in categories_res:
            categories.append(category_res[3])

        mycursor.execute(queries.SQL_SELECT_CITY_PRODUCT, (product_id,))
        cities_res = mycursor.fetchall()
        cities = []
        for city_res in cities_res:
            cities.append(city_res[3])

        mycursor.execute(queries.SQL_SELECT_STORE_PRODUCT, (product_id,))
...
class Product:
    ...
    def __str__(self):
        ...
        return (
            f"{self.id}, {self.name}, {self.code}, {self.brand}, "
